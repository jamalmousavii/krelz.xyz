...
router.post('/register', async (req, res) => {
  try {
    const { wallet_address, gpu_model, ram, cpu } = req.body;
    ...
    // ایجاد ماینر
    const result = await pool.query(
      'INSERT INTO miners (wallet_address, gpu_model, ram, cpu) VALUES ($1, $2, $3, $4) RETURNING *',
      [wallet_address, gpu_model, ram, cpu]
    );
...
router.put('/:id/heartbeat', async (req, res) => {
  try {
    const { id } = req.params;
    const { status, gpu_usage, ram_usage, tasks_completed } = req.body;
